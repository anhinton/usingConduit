...
The source script creates an 'R data frame' object, `rentalListings`,
...
This module has one input, an 'R data frame' `rentalListings`, and one
source from the file
[chooseSuburbs.R](aucklandRents/src/chooseSuburbs.R). This source
replaces the `rentalListings` object with a smaller data frame of
rental listing from three specific suburbs. We will name
`rentalListings` as an internal output:
...
requires an "R data frame" object, `rentalListings`. We create the module
`plotGraph` with the following code:
...
Two more pipes are required, connecting "rentalListings" from
`nameSuburbs` to "rentalListings" in `chooseSuburbs`, connection
"rentalListings" from `chooseSuburbs` to "rentalListings" in
`boxplotBySuburbs`.
...
<span class="ann">TODO: 2015-02-17: UP TO HERE</span>
